...
//  Loop
//
//  The interactive glucose chart, ported from LoopFollow
//  (LoopFollow/Charts/BGChartView.swift). Rendering and gesture handling are
//  unchanged; only the three places that reached into LoopFollow's own
//  settings and unit formatting were repointed at `BGChartSettings` /
//  `BGChartGlucoseDisplay`.
//
...
private struct MainBGChart: View {
    ...
    private var timeZoneForAxis: TimeZone {
        BGChartSettings.shared.axisTimeZone ?? .current
    }
